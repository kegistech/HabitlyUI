// src/screens/ReflectionTime/index.tsx
...
import { updateReflectionTimeAPI } from '../../services/apiendpoints';

type Props = NativeStackScreenProps<RootStackParamList, 'ReflectionTime'>;
...
const ReflectionTimeScreen: React.FC<Props> = ({ navigation }) => {
  // Time selection state (Default: 07:30 PM)
  ...
  // FlatList Refs for initial scroll alignment
  ...
  // Format 12-hour selection into C# TimeOnly format ("HH:mm:ss")
  ...
  const handleContinue = () => {
    ...
    const payload = {
      reflectionTime: formattedTimeOnly,
    };

    setIsLoading(true);
    putApi(
      updateReflectionTimeAPI,
      payload,
      (response: any) => {
        setIsLoading(false);
        if (response && (response.succeeded || response.isSuccess || response.status === 200)) {
          ...
          setErrorMessage(response?.message || 'Failed to update reflection time. Please try again.');
        }
      },
      (error: any) => {
        setIsLoading(false);
      ...
      <View style={styles.columnContainer}>
        <FlatList
          ...
          contentContainerStyle={{
            paddingVertical: ITEM_HEIGHT, // Centers first and last items
          }}
        ...
        >
          ...
          <View style={styles.headerContainer}>
            <View style={styles.tagBadge}>
              <Text style={styles.tagBadgeText}>EVENING REFLECTION</Text>
            </View>
            ...
            <Text style={styles.subText}>
              A quick evening check-in helps celebrate wins and stay consistent.
            </Text>
          </View>
...
export default ReflectionTimeScreen;